/**
 * CSS style constants and computed style functions
 * Mirrors browser default CSS styles
 */

// Numeric index to CSS property name mapping (0-280)
const cssPropertyNames: Record<number, string> = {
  0: 'animation-delay',
...
// Default CSS style values for computed styles
...
// Build full style object with both numeric and named keys
export const cssStyles: Record<string | number, string> = {
  ...cssPropertyNames,
  ...defaultStyles,
};

/**
 * Get computed style for an image element
 */
export function getImageComputedStyle(image: HTMLImageElement): Record<string, string> {
...
/**
 * Get computed style for a canvas element
 */
export function getCanvasComputedStyle(canvas: HTMLCanvasElement): Record<string, string> {
  // Try to get bounding rect if available, otherwise use canvas dimensions
  let rect: { width: number; height: number };

  if (typeof canvas.getBoundingClientRect === 'function') {
    rect = canvas.getBoundingClientRect();
  } else {
    rect = { width: canvas.width || 0, height: canvas.height || 0 };
  }
